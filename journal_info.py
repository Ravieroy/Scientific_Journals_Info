...
def format_response(search_results):
    ...
    ''' %(journal_name, country_name, rank, sjr, h_index, total_docs_in_three, total_cites_in_three, citation_per_doc ,joined_list)
...
def get_data(journal_name):
    if journal_name != '': # This if block checks if the entry field is empty.
        try:
            ...
            try:
                data = pd.read_csv(fileName,sep=';', index_col=False, dtype='object') # Read csv data
                list_of_items= ['Rank','Sourceid','Title','SJR','H index','Total Docs. (3years)','Total Cites (3years)','Cites / Doc. (2years)','Country','Publisher','Categories']
                data = data[list_of_items]
                search_results = data.loc[data['Title'].str.contains(journal_name , case=False)]
                label['text'] = format_response(search_results)
                return search_results
            except FileNotFoundError:
                messagebox.showerror('FileNotFoundError', 'journal_databse.csv not found. Press Update DB button')
...
def search_command(journal_name):
    ...
    if journal_name != '': # This if block checks if the entry field is empty.
        ...
        try:
            ...
            list_of_items= ['Rank','Sourceid','Title','SJR','H index','Total Docs. (3years)','Total Cites (3years)','Country','Publisher','Categories']
            data = data[list_of_items]
...
def update_command(): #This will download the csv file if not already present. If it is present it will overwrite it
    try:
        ...
        messagebox.showerror('Connection Error', 'Looks like you are not connected to internet.')
...
button = tk.Button(top_frame, text='Get Results',font=('courier',8),command=lambda: get_data(entry.get()))
...
#----Search Button-----
search_button = tk.Button(root, text='Search',bg='gray',command=lambda: search_command(entry.get()))
